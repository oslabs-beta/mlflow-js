...
class ModelManager {
    ...
    async createRegisteredModelWithVersion(name, versionSource, versionRun_id) {
        try {
            ...
            const response = await this.modelVersion.createModelVersion(name, versionSource, versionRun_id);
            return response;
        }
        catch (error) {
     ...
     * @throws {ApiError | Error} If request fails
     */
    async updateRegisteredModelDescriptionAndTag(name, tagKey, tagValue, description) {
        try {
            await this.modelRegistry.setRegisteredModelTag(name, tagKey, tagValue);
            const response = await this.modelRegistry.updateRegisteredModel(name, description);
            return response;
        }
        catch (error) {
            if (error instanceof ApiError) {
    ...
    async updateAllLatestModelVersion(name, alias, description, key, value) {
        try {
            ...
            if (!data) {
                ...
                const response = await this.modelVersion.updateModelVersion(name, version, description);
                return response;
            }
        }
        catch (error) {
     ...
     * @throws {ApiError | Error} If request fails
     */
    async updateAllModelVersion(name, version, alias, key, value, description) {
        try {
            await this.modelRegistry.setRegisteredModelAlias(name, alias, version);
            await this.modelVersion.setModelVersionTag(name, version, key, value);
            const response = await this.modelVersion.updateModelVersion(name, version, description);
     ...
     * @throws {ApiError | Error} If request fails
     */
    async createModelFromRunWithBestMetric(experiment_ids, filterMetric, metricMinOrMax, modelName) {
        try {
            const data = await this.runClient.searchRuns(experiment_ids, `metrics.${filterMetric} != -99999`);
            ...
            await this.modelVersion.createModelVersion(modelName, bestRun.info.artifact_uri, bestRun.info.run_id);
            return;
        }
        catch (error) {
            if (error instanceof ApiError) {
