...
class ModelVersionClient {
     ...
     */
    async createModelVersion(modelName, source, run_id, tags, run_link, description) {
        // fire off a post request to create the model versions
        const { response, data } = await apiRequest(this.baseUrl, 'model-versions/create', {
            method: 'POST',
            body: { name: modelName, source, run_id, tags, run_link, description },
        });
        // is response ok? else throw error
        if (!response.ok) {
            throw new ApiError(`Error creating model version: ${data.message || response.statusText}`, response.status);
        }
    ...
    async getModelVersion(modelName, version) {
        ...
        const { response, data } = await apiRequest(this.baseUrl, 'model-versions/get', {
            method: 'GET',
            params: { name: modelName, version },
        });
        // is response ok? else throw error
        if (!response.ok) {
            throw new ApiError(`Error fetching model version: ${data.message || response.statusText}`, response.status);
        }
        // return the model version obj
        return data.model_version;
    }
    ...
    async updateModelVersion(modelName, version, description) {
        ...
        const { response, data } = await apiRequest(this.baseUrl, 'model-versions/update', {
            method: 'PATCH',
            body: { name: modelName, version, description },
        });
        // is response ok? else throw error
        if (!response.ok) {
            throw new ApiError(`Error updating model version: ${data.message || response.statusText}`, response.status);
        }
     ...
     * @throws {ApiError} If the API request fails
     */
    async searchModelVersions(filter, maxResults, order_by, page_token) {
        const body = {};
        if (filter) {
            body.filter = filter;
        }
        ...
        const { response, data } = await apiRequest(this.baseUrl, 'model-versions/search', {
            method: 'GET',
            params: body,
        });
        // is response ok? else throw error
        if (!response.ok) {
            throw new ApiError(`Error searching model versions: ${data.message || response.statusText}`, response.status);
        }
    ...
    async getDownloadUriForModelVersionArtifacts(modelName, version) {
        ...
        const { response, data } = await apiRequest(this.baseUrl, 'model-versions/get-download-uri', {
            method: 'GET',
            params: { name: modelName, version },
        });
        // is response ok? else throw error
        if (!response.ok) {
            throw new ApiError(`Error fetching download uri: ${data.message || response.statusText}`, response.status);
        }
     ...
     * @throws {ApiError} If the API request fails
     */
    async transitionModelVersionStage(modelName, version, stage, archive_existing_versions) {
        // fire off a post request to transition the model version stage
        const { response, data } = await apiRequest(this.baseUrl, 'model-versions/transition-stage', {
            method: 'POST',
            body: { name: modelName, version, stage, archive_existing_versions },
        });
        // is response ok? else throw error
        if (!response.ok) {
            throw new ApiError(`Error transitioning model version stage: ${data.message || response.statusText}`, response.status);
        }
        // return the updated model version obj
        return data.model_version;
    }
    ...
    async setModelVersionTag(modelName, version, key, value) {
        ...
        const { response, data } = await apiRequest(this.baseUrl, 'model-versions/set-tag', {
            method: 'POST',
            body: { name: modelName, version, key, value },
        });
        // is response ok? else throw error
        if (!response.ok) {
            throw new ApiError(`Error setting model version tag: ${data.message || response.statusText}`, response.status);
        }
    ...
    async deleteModelVersionTag(modelName, version, key) {
        ...
        const { response, data } = await apiRequest(this.baseUrl, 'model-versions/delete-tag', {
            method: 'DELETE',
            body: { name: modelName, version, key },
        });
        // is response ok? else throw error
        if (!response.ok) {
            throw new ApiError(`Error deleting model version tag: ${data.message || response.statusText}`, response.status);
        }
        // return nothing, just resolve
        return;
    }
    ...
    async deleteModelVersion(modelName, version) {
        ...
        const { response, data } = await apiRequest(this.baseUrl, 'model-versions/delete', {
            method: 'DELETE',
            body: { name: modelName, version },
        });
        // is response ok? else throw error
        if (!response.ok) {
            throw new ApiError(`Error deleting model version: ${data.message || response.statusText}`, response.status);
        }
        // return nothing, just resolve
        return;
    }
...
//# sourceMappingURL=ModelVersionClient.js.map